...
$timestamp= scandir("/srv/ts-yt-dl/$userid/videos/");//$userid/vidoes/");
...
echo "<body>
		...
		<form action=\"./ts_dvd.php\" method=\"POST\">
			<table id=\"media_table\">
				<tr>
					...
					<th class=\"small_cell\"><h4>User ID</h4></th>
	...
	for ($t = 0; $t < $timestamp_count; $t++) {
		...
		if ($timestamp[$t] != "." && $timestamp[$t] != "..") {
			...
			$video = scandir("/srv/ts-yt-dl/$userid/videos/".$timestamp[$t]);
				...
				if ($video[$v] != "." && $video[$v] != "..") {
					...
					if ($ext != 'jpg' && $video[$v] != 'log') {
						echo "<tr>";
							...
							echo "<td class=\"small_cell\">$userid</td>";
...
echo "</table>";
echo "		<input class=\"download_submit\" type=\"submit\" value=\"Download\">
	</form>";
